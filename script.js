...
const playWatermark = document.querySelector(".play-watermark")
...
function updateCatchUpLine(){
    ...
    if(((syncVideo.currentTime - video.currentTime)/playbackPace)/(1 - 1/playbackPace) > video.duration - syncVideo.currentTime){
      ...
      catchUpTimeText.style.animation = "blinker 0.75s step-start infinite"
    }
    else{
    	catchUpTimeText.style.color = "grey"
      catchUpTimeText.style.animation = "none"
    }
}
...
function togglePlayPause(){
	if(started){
        ...
        playWatermark.style.display = "none"
        ...
        playWatermark.style.display = "block"
...
function toggleShowTranscript(){
	if(transcriptIsShown){
		...
		transcriptButton.children[1].style.display = "block"   //show the red line
...
//when the syncvideo ends open the quiz window
syncVideo.addEventListener("ended", openQuiz)
function openQuiz(){
  video.paused = true
